...
#             start = i*batch_size
    ...
    def fit_test(self):
        ...
        Y_pred_test = np.argmax(outputs_test[1][-1], axis=0)
...
sweep_id_tr = wandb.sweep(sweep_config_tr,project=wandb_project, entity=wandb_entity)

tuned_models = []
# wandb.init(project="Testing", entity="dl_research")
def train():
    with wandb.init() as run:
        ...
        for epoch in range(epochs):
            train_acc,train_loss,val_acc,val_loss = model.fit_test()  # model training code here
            metrics = {
            "accuracy":train_acc,
                "loss":train_loss,
...
wandb.agent(sweep_id_tr, function=train, count=1)

print("Final Scores: \nModel Hyperparameters: {}\nAccuracy: {}\nLoss: {}\nTest Accuracy: {}\nTest Loss {}".format(tuned_models[0]['model'], tuned_models[0]['accuracy'], tuned_models[0]['loss'], tuned_models[0]['test_accuracy'], tuned_models[0]['test_loss']))

def best_model(tuned_models):
  sorted_list = sorted(tuned_models, key = lambda d : d['test_accuracy'],reverse=True)
...
wandb.log({"Confusion Matrix Best": wandb.Image(image)})
